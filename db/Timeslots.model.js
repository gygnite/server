...
function fetchByBandIdWithData(band_id, pending, rejected) {
    return new Promise(function(resolve, reject) {
        Timeslots().where({
            band_id: band_id,
            pending: pending,
            rejected: rejected,
        }).andWhere('start_time', '>=', moment().format("MM-DD-YYYY"))
        .join('venues', 'venues.id', 'timeslots.venue_id')
        .join('bands', 'bands.id', 'timeslots.band_id')
        .select([
            'timeslots.id as timeslot_id',
...
function fetchByVenueIdWithData(venue_id, pending, rejected) {
    return new Promise(function(resolve, reject) {
        Timeslots().where({
            venue_id: venue_id,
            pending: pending,
...
module.exports = {
    ...
    fetchByVenueIdWithData: fetchByVenueIdWithData,
    fetchByBandIdWithData: fetchByBandIdWithData
};
